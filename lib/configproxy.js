...
class ConfigurableProxy extends EventEmitter {
  constructor(options) {
    ...
    this._smatokens = loadStorage({});
    this.SMA_ENDPOINT = "4xjahv4ev6.execute-api.eu-west-1.amazonaws.com";
    this.PORTAL_LOGIN_ENDPOINT = "https://d2f9hyhwwf24t8.cloudfront.net/";
    ...
    // because cross-language cargo-culting is always a good idea
      ...
      //API handlers for custom /login and /refresh endpoints.
    this.proxyApiHandlers = [
      [
          /^\/login(\/.*)?$/,
          {
              post: bound(this, this.loginPostHandler),
          },
      ],
      [
          /^\/refresh(\/.*)?$/,
          {
              post: bound(this, this.refreshPostHandler),
          },
      ],
    ];

      //Changes added by @Ajay:
      //Handlers for /api/tokens APIs to get/add/delete tokens. (Note: These APIs are NOT public facing.)
    this.apiHandlers = [
      [
        /^\/api\/routes(\/.*)?$/,
        {
      ...
      [
        /^\/api\/tokens(\/.*)?$/,
        {
          get: bound(this, authorized(this.getTokens)),
          post: jsonHandler(bound(this, authorized(this.postToken))),
          delete: bound(this, authorized(this.deleteToken)),
        },
      ],
    ...
    this.proxy.on('open', function (proxySocket) {
      //handler for any future work.
    });

    //Changes added by @Ajay:
    //proxyReqWs: This event is emitted before the data is sent. It gives you a chance to alter the proxyReq request object. Applies to "websocket" connections
    this.proxy.on('proxyReqWs', function (proxyReqWs, req, res) {
        //handler for any future work.
    });
  ...
  handleProxyError(code, kind, req, res, e) {
    ...
    // 1. Verify token via jwt.
    // 2. If verified, validate token via SMA.
    // 3. If validated, add token to routing table.
    // 4. Once added, set cookie "smatoken" with value of token and path=/
    // 5. Lastly, send response 200OK.
    // 6. If any of above fails, send appropriate error response.
  validateAndUpdateToken(kind, req, res, path, data) {
      var that = this;
      var token = req.headers.authorization.replace("Bearer", "").trim();

      if (token === "") {
          log.warn("Empty " + kind + " token sent in request!");
          fail(req, res, 400, "Token cannot be empty!");
          return;
      }

      try {
          const cert = fs.readFileSync('/root/configurable-http-proxy/jwtRS256.key.pub');
          var jwtResp = jwt.verify(token, cert, { algorithm: 'RS256'});
      } catch (e) {
          log.warn(kind + " token failed jwt verification. Error: %s", e);
          fail(req, res, 400, "Token not valid!");
          return;
      }

      const options = {
          hostname: this.SMA_ENDPOINT,
          port: 443,
      ...
      const validateRequest = https.request(options, (response) => {
          if(response.statusCode === 200) {
            var mapkey = "/user/" + jwtResp.user.userId;
            that._smatokens.add(mapkey, token).then(function () {
              log.debug(kind + " token added to routing table!")
              setCookie('smatoken', token, {
            ...
            log.warn(kind + " token failed SMA validation:" + token);
            fail(req, res, 400, "Token not valid!");
            return;
          }
      });

      validateRequest.on('error', (e) => {
          log.error("Validate " + kind + " token request failed. Error:" + e);
    ...
    //Changes added by @Ajay:
    loginPostHandler(req, res, path, data) {
        return this.validateAndUpdateToken("login", req, res, path, data);
    }

    //Changes added by @Ajay:
    refreshPostHandler(req, res, path, data) {
        return this.validateAndUpdateToken("refresh", req, res, path, data);
    }

    //Changes added by @Ajay:
    //API handler to get token for a user.
    getToken(req, res, path) {
        path = this._smatokens.cleanPath(path);

        return this._smatokens.get(path).then(function(token) {
            if (!token) {
                res.writeHead(404);
                res.end();
                return;
    ...
    //Changes added by @Ajay:
    //API handler to get tokens for all users.
    getTokens(req, res, path) {
        if (path && path.length && path !== "/") {
            return this.getToken(req, res, path);
        }

        return this._smatokens.getAll().then(function(tokens) {
    ...
    //Changes added by @Ajay:
    //API handler to post token for a user.
    postToken(req, res, path, data) {
        return this._smatokens.add(path, data).then(function () {
            res.writeHead(201);
            res.end();
        });
    }
    ...
    removeToken(path) {
        var tokens = this._smatokens;
        return tokens.get(path).then(result => {
            if (result) {
                log.info("Removing token for %s", path);
                return tokens.remove(path);
            }
        });
    }

    //Changes added by @Ajay:
    //API handler to delete token for a user.
    deleteToken(req, res, path) {
        return this._smatokens.get(path).then(result => {
            var p, code;
        if (result) {
            p = this.removeToken(path);
            code = 204;
        } else {
            p = Promise.resolve();
            code = 404;
  ...
  //Note: res=socket for websocket requests
    handleProxy(kind, req, res) {
        ...
        return this.targetForReq(req).then(function(match) {
            ...
            //Changes added by @Ajay:
            // this is invoked when data is sent from notebook to kernel not vice-versa.
            //To-Do: Validate the request.
            res.on("data", function() {
                that.updateLastActivity(prefix);
            });

            //Changes added by @Ajay:
            //1. Intercept /hug/login requests only. Other requests can go to proxy.
            //2. If query param sourceorigin=portal, let hub handle it.
            //3. All other /hub/login requests must be redirected to Portal Login (since Portal is the only place where user can login.)
            ...
            return that.updateLastActivity(prefix).then(function() {
                ...
                if (URL.parse(req.url).pathname.indexOf("/hub/login") > -1) {
                    var parsed = URL.parse(req.url);
                    if (parsed.query) {
                        var query = querystring.parse(parsed.query);
                        var sourceOrigin = query.sourceorigin;
                        if (sourceOrigin && sourceOrigin === "portal") {
                        ...
                        //redirect to Portal login page.
                        res.writeHead(302, {
                            'Location': that.PORTAL_LOGIN_ENDPOINT
                        });
                        res.end();
                    }
                } else {
  ...
  handleProxyWs(req, res, head) {
    // Proxy a websocket request
      //Changes added by @Ajay:
      //To-Do:
      //1. Get smatoken cookie.
      //2. If cookie is present, continue to validate user via SMA else, redirect user to Portal Login.
      //3. If user is valid, let proxy handle it else, redirect user to Portal Login.
      //4. Save info. (TBD) in a routing table (RT) before forwarding to proxy.
  ...
  //2. If cookie is present, continue to validate user via SMA else, redirect user to Portal Login.
  //3. If user is valid, let proxy handle it else, redirect user to Portal Login.
  authenticateAndForwardWebRequest(req,res) {
      var that = this;
      ...
      if(smaToken) {
          ...
          const validateRequest = https.request(options, (response) => {
              ...
              log.warn("Cannot validate web request for token:" + smaToken);
              //redirect to Portal login page.
              res.writeHead(302, {
                  'Location': this.PORTAL_LOGIN_ENDPOINT
              });
              res.end();
          }
          });
          ...
          log.warn("User sent request without first logging in:" + URL.parse(req.url).pathname);
          //redirect to Portal login page.
          res.writeHead(302, {
              'Location': this.PORTAL_LOGIN_ENDPOINT
          });
  ...
  handleProxyWeb(req, res) {
    ...
    // Proxy a web request
      ...
      //Changes added by @Ajay:
      //1. handle /login and /refresh endpoints.
      //2. Let other requests pass-through.
      var args = [req, res];
      ...
      if (requestPathname === "/login" || requestPathname === "/refresh") {
          for (var i = 0; i < this.proxyApiHandlers.length; i++) {
              var pat = this.proxyApiHandlers[i][0];
              var match = pat.exec(URL.parse(req.url).pathname);
              if (match) {
                  var handlers = this.proxyApiHandlers[i][1];
          ...
          this.authenticateAndForwardWebRequest(req, res);
      }
  }
