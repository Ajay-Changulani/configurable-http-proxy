  ...
  constructor(options) {
    ...
    this.proxyApiHandlers = [
      [
          /^\/login(\/.*)?$/,
          {
              post: jsonHandler(bound(this, this.loginPostHandler)),
          },
      ],
      [
          /^\/refresh(\/.*)?$/,
          {
              post: jsonHandler(bound(this, this.refreshPostHandler)),
          },
      ],
    ];
...
// Changes below added by @Ajay:


// 1. verify token
// 2. validate token
// 3. add token to routing table
// 4. set cookie
// 5. send response 200
  loginPostHandler(req, res, path, data) {
      ...
      if (userToken === "") {
          log.warn("Bad POST login data from Portal: %s", JSON.stringify(data));
          fail(req, res, 400, "Token cannot be empty!");
      }
      try {
          const cert = fs.readFileSync('/root/configurable-http-proxy/jwtRS256.key.pub');
          var jwtResp = jwt.verify(userToken, cert, { algorithm: 'RS256'});
      } catch (e) {
          log.error("User token not valid %s", e);
          fail(req, res, 400, "User token not valid!");
      }

      const options = {
          hostname: '4xjahv4ev6.execute-api.eu-west-1.amazonaws.com',
          port: 443,
      ...
      const validateRequest = https.request(options, (response) => {
        if(response.statusCode === 200) {
          ...
          this._smatokens.add(mapkey, userToken).then(function () {
              log.info("User token added to routing table!")
              setCookie('smatoken', userToken, {
          ...
          log.warn("Invalid user token:" + userToken);
          fail(req, res, 400, "Invalid user token!");
        }
      });
  ...
  //res=socket for websocket requests
    handleProxy(kind, req, res) {
        ...
        return this.targetForReq(req).then(function(match) {
            ...
            return that.updateLastActivity(prefix).then(function() {
                ...
                //@Ajay
                //1. Intercept /hug/login requests.
                //2. If sourceorigin flag is present, let it pass through
                //3. else, redirect to portal.
                if (URL.parse(req.url).pathname.indexOf("/hub/login") > -1) {
                    ...
                    if (parsed.query) {
                        ...
                        if (sourceOrigin && sourceOrigin === "portal") {
                            ...
                            res.writeHead(302, {
                                'Location': 'https://d2f9hyhwwf24t8.cloudfront.net/'
                            });
                            res.end();
                        }
                    } else {
  ...
  authenticateAndForwardWebRequest(req,res) {
      ...
      //@Ajay:
      //1. If cookie is set, continue validating it.
      //2. If cookie is valid, forward request else, redirect to Portal login.
      //3. If cookie is not set, redirect to Portal login.
      if(smaToken) {
          const options = {
              hostname: '4xjahv4ev6.execute-api.eu-west-1.amazonaws.com',
              port: 443,
  ...
  handleProxyWeb(req, res) {
      log.info("********** WEB-request-invoked:" + URL.parse(req.url).pathname);
      this.statsd.increment("requests.web", 1);

    // Proxy a web request
      //@Ajay:
      //1. handle /login and /refresh endpoints.
      //2. Let other requests just pass-through.
      var args = [req, res];
