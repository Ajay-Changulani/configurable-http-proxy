...
class ConfigurableProxy extends EventEmitter {
  ...
  // Change added by @Ajay: Methods to get, post and delete sma token.


    // authenticateUser(url) {
    //     var path = decodeURIComponent(URL.parse(url).pathname);
    //     var tokens = this._smatokens.getTarget(path);
    //     var user_token = tokens.data.token;
    //     var user_token_ts = tokens.data.ts;
    //     // const var options = {
    //     //       host: '4xjahv4ev6.execute-api.eu-west-1.amazonaws.com/development/validate',
    //     //       path: '/db/data',
    //     //       method: 'GET',
    //     //       headers: {
  ...
  //1. invoke sma /validate to validate the token.
  //2. return true if 200 else false.
  validateUserToken(userToken) {


  }
    //1. verify this token using jwt.io
    // 2. send this token to SMA
    //2. return true if valid otherwise false
  authenticateUserToken(userToken) {
      var returnValue = "";
      const cert = fs.readFileSync('jwtRS256.key.pub');
      try {
          const validToken = jwt.verify(userToken, cert, { algorithm: 'RS256'});
          this.validateUserToken(userToken).then(function (result) {
              if (result) {
                  return Promise.resolve(validToken.user.userId);
              } else {
                  return Promise.resolve("");
              }
          });
      } catch (e) {
          log.error("User token not valid %s", userToken);
          return Promise.resolve("");
      }
  }

    //1. Authenticate user
    //2. add user info to Rt
    //3. set cookie
    //4. redirect to hub
  loginPostHandler(req, res, path, data) {
    ...
    if (userToken === "") {
        ...
        fail(req, res, 400, "Must specify 'token' as string");
        return;
    }

    this.authenticateUserToken(userToken).then(function (userId) {
        if (!userId) {
            //redirect-to-Portal-login-page.
        }

        //key is /user/dna
        this.mapkey = "/user/" + data.user; // @Ajay-Alert: user may have been extracted from the token
        this._smatokens.add(mapkey, data).then(function () {
  ...
  handleProxyWeb(req, res) {
      log.info("------------------------> WEB-request-invoked");
    // Proxy a web request
      //@Ajay: Check if request is /login or /refresh
      ...
      //@Ajay: If not, handle as default request and send to proxy.
      this.statsd.increment("requests.web", 1);
      return this.handleProxy("web", req, res);
  }
